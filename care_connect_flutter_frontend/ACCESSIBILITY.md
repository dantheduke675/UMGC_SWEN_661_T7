...
Re-run on the same emulator to extend the walk down the sign-up path, which
the first pass never entered: **Create account**, and the two Face ID screens
behind it. The committed transcript is that run, stamped 2026-09-22 01:42 UTC;
the eight screens the first pass had covered came back byte-identical.

The full transcript is committed at **`evidence/talkback-transcript.txt`** and
regenerated by `tool/talkback_walk.sh`. It records, for all eleven screens,
...
**One screen is read by a different tool, and says so.** `uiautomator dump`
waits for the window to go idle before it will produce a tree. The Face ID
sign-in screen spins a `CircularProgressIndicator` for as long as it is on
screen, driven by Flutter's own ticker, so that window never goes idle and
uiautomator can never dump it — not on a retry, and not with Android's
animator scale at zero, which does not reach Flutter's ticker. That screen is
therefore read with `maestro hierarchy`, which reads the same platform
accessibility tree through `AccessibilityNodeInfo` but does not require the
window to stop moving. The transcript names the reader under that screen.
The one state Maestro does not expose is the password flag, and there is no
password field on that screen.

Worth knowing if this is ever re-run: `uiautomator dump` **exits 0 when that
idle wait times out**, having written nothing. The script used to let the
previous screen's file stand in such a case, which silently recorded one
screen's nodes under another screen's heading — wrong evidence that looks
exactly like right evidence. It now deletes the file first and treats a
non-empty pull as the only proof of a successful read.

So the transcript supports, as fact:

- every control has a non-empty accessible name (**0 unnamed of 154 nodes**);
...
| | |
|---|---|
| Screens walked | 11 (Landing, Create account, Face ID setup, Face ID sign-in, Sign in, Today, Medications, Schedule, Symptoms, Messages, Account) |
| Nodes TalkBack can stop on | 154 |
...
**One defect the sign-up screens added, not yet fixed.** On Face ID sign-in
the spinner reaches the platform tree as a progress node whose entire
accessible name is a number:

```
  5. [ProgressBar] 75
```

The node arrives as `android.widget.ProgressBar` with a content description of
`75` and no label of its own. Nothing on that screen is 75% complete — the
`CircularProgressIndicator` inside `AuthSpinner` (`lib/widgets.dart`) is
indeterminate and spins for as long as the screen is up — so TalkBack is handed
a figure with nothing to say what it refers to. The screen is not unusable:
the line beside it, `Looking for your face…`, is what carries the meaning and
is named correctly. But the progress node contributes an announcement that
means nothing. Either excluding the indicator from semantics and letting that
line speak for it, or giving it a real label, would settle it. Recorded rather
than done because this pass was scoped to capturing the screens, not changing
them.
...
- **`Dark mode` is announced next to `Appearance: Dark mode`** on Account: the
  ...
  a working tile.
- **`Caregiver` is announced as disabled** on Create account. `AuthRoleTile`
  for the caregiver role is built with no `onPressed`, so it reaches the tree
  disabled, and TalkBack says `Caregiver, button, disabled` rather than going
  silent — the correct behaviour for a control that cannot be used. Recorded
  because the transcript now shows a reader meeting a role they cannot pick,
  and the cause is the caregiver path not being wired up rather than anything
  about the button. `Care recipient`, beside it, works.
...
Touch exploration is the part the walk depends on, and it does not always
come up with the service; check it reports `touchExplorationEnabled=true` in
`adb shell dumpsys accessibility` before trusting a run.

The Face ID sign-in screen needs the Maestro CLI on hand as well, for the
reason given above. The script looks for it at `~/.maestro/bin/maestro` and
takes a `MAESTRO=` override.

Traps worth knowing, all of which produced wrong answers before they were
understood:
...
- **`uiautomator dump` exits 0 when it fails.** On a window that never goes
  idle it prints `ERROR: could not get idle state` and writes nothing, while
  still reporting success. A script that pulls the device-side file without
  clearing it first gets the *previous* screen's tree and files it under the
  new screen's heading. Delete the file first; treat a non-empty pull as the
  only evidence of a read.
- **Do not name a shell variable `TMP`.** Windows exports `TMP`, so Git Bash
  inherits it already exported and assigning it re-exports the new value to
  every child process. The script's dump path was once `TMP=./tbdump.xml`,
  which handed Maestro's JVM a temp directory of `./tbdump.xml` — the JVM
  duly created it, and the dump path became a directory. It is `TREE` now.
- **`python` here is Windows python, which does not understand MSYS paths.**
  A path like `/tmp/x` is read as drive-relative, so bash can see a file that
  python then cannot open. The script keeps its scratch files relative to the
